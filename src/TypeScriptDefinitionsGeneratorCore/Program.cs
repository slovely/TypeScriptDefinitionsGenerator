...
namespace TypeScriptDefinitionsGeneratorCore
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var result = Parser.Default.ParseArguments<Options>(args.ToList()).
        ...
        private static void GenerateTypeScriptContracts(Options options)
        {
            ...
            foreach (var assemblyName in options.Assemblies)
            {
                ...
                var controllers = assembly.GetTypes().Where(t => typeof(Controller).IsAssignableFrom(t));
        ...
        private static void GenerateWebApiActions(Options options)
        {
            ...
            {
                var assembly = Assembly.LoadFrom(assemblyName);
                var controllers = assembly.GetTypes().Where(t => typeof(Controller).IsAssignableFrom(t)).OrderBy(t => t.Name);
        ...
        private static List<ActionParameterInfo> GetActionParameters(MethodInfo action)
        {
            ...
            foreach (var parameterInfo in parameters)
            {
                ...
                var fromUri = parameterInfo.GetCustomAttributes<FromQueryAttribute>().FirstOrDefault();
                if (fromUri != null)
                {
